...
const ITEMS_PER_PAGE = 18; // Số lượng video mỗi trang
...
window.openInfoPopup = function (movieId) {
    ...
    infoPopup.innerHTML = `
        ...
        <div class="info-item"><strong>Diễn viên:</strong> ${movie.actor || 'Đang cập nhật'}</div>
...
function initializeHomepage() {
    ...
    featuredMovies.forEach((movie) => {
        ...
        // --- LOGIC MỚI ---
        // Sử dụng trực tiếp 'poster-slide' làm ảnh nền
        // Có kiểm tra để đảm bảo thuộc tính tồn tại
        if (movie['poster-slide']) {
            slide.style.backgroundImage = `url(${movie['poster-slide']})`;
        } else {
            // Fallback nếu 'poster-slide' không có
